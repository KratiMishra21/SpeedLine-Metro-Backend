...
export const getLiveMapData = async (req, res) => {
  try {
    ...
    }).populate('station', 'name stationId').sort({ createdAt: -1 });
    ...
    // Calculate average crowd level with MORE WEIGHT on recent reports
    const liveStationData = stations.map(station => {
      ...
      if (stationReports && stationReports.reports.length > 0) {
        ...
        const sortedReports = stationReports.reports.sort((a, b) => 
          new Date(b.createdAt) - new Date(a.createdAt)
        );
        
        // Give MORE WEIGHT to the most recent reports (last 10 minutes)
        const tenMinsAgo = new Date(Date.now() - 10 * 60 * 1000);
        const veryRecentReports = sortedReports.filter(r => new Date(r.createdAt) >= tenMinsAgo);
        ...
        if (veryRecentReports.length > 0) {
          ...
          const olderReports = sortedReports.filter(r => new Date(r.createdAt) < tenMinsAgo);
...
export const getStationDetails = async (req, res) => {
  try {
    ...
    if (recentReports.length > 0) {
      const tenMinsAgo = new Date(Date.now() - 10 * 60 * 1000);
      const veryRecentReports = recentReports.filter(r => new Date(r.createdAt) >= tenMinsAgo);
      ...
      if (veryRecentReports.length > 0) {
        ...
        const olderReports = recentReports.filter(r => new Date(r.createdAt) < tenMinsAgo);
